...
class graph
{
	...
	private:
		...
		int **edgeArr;
		int numnodes;
		int *visited = new int[numnodes]; // array to keep track of visited spots
};
...
*/
{
	int *visited = new int[n+1]; // array to keep track of visited spots
    int queue[n+1]; // array to store visited order
	int a = 0, b = 1; // matrix vars
	int i;
	
	for (i=1;i <=n ; i ++)   // set all to 0
	{
	     queue[i] = 0;
	     visited[i] = 0;
    }
    ...
    while( a != b ) 
    {
        ...
        for( int j = 1; j <= n; j++) // loop to track visited nodes
            if( graphArr[i][j] != 0 && visited[j] != 1 ) // if the matrix isn't 0 and hasn't been visited
            {
              queue[++b] = j; // increment queue position with rear var and make that what's in the j counter
...
// doesn't work ...
void graph::dfs( int x )           // depth-first node order
/*
...
*/
{
	...
	for ( j = 0; j < numnodes; j++ ) // counter 
	{
		if ( !visited[j] && graphArr[x][j] ) // if position not visited move position in matrix
		{
			dfs(j); // recursive call with value
		}
	}
}
