...
graph::graph()
{
	graphArr = new int*[100]; // create array/pointer
	for ( int i = 0; i < 100; ++i )
	{
		graphArr[i] = new int[100]; // create array of pointers
	}
}
...
graph::graph( int n )
{
	...
	for ( int i = 0; i < n; ++i )
	{
		graphArr[i] = new int[n]; // create array of pointers
	}
}
...
graph::graph( int n, int e )
{
	...
	edgeArr = new int*[e]; // create the edge array/pointer for what, I don't know
	for ( int i = 0; i < e; ++i ) 
	{
		edgeArr[i] = new int[e]; // create array of pointers
	}
}
...
void graph::mprint(int n)
{
	...
	for ( i = 0; i < n; i++ ) // numnode (vertices)
	{
		cout << i+1 << "|	"; // build row headers
		for ( j = 0; j < n; j++ ) // numnode (verticies)
		{
			cout <<  graphArr[i][j] << "	";
...
void graph::fill( ifstream& file )
{
	...
	while( true )
	{
		...
		graphArr[firstVert - 1][secondVert - 1] = weight; // add weighted edge value into the matrix
		graphArr[secondVert - 1][firstVert - 1] = weight; // other direction/because undirected
	}
}
