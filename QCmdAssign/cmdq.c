...
{
	int a, b, x, len, sum, t = 1; // vars a and b for command file, x for deq() return, len and sum for clearning queue, and t for printing queue
	char *buff = malloc(100 * sizeof(char)); // initialize memory buffer for cmd file
	FILE *cmdFile; // initialize file pointer
	queue s;
	s = makeq();


	if (argc < 2 || argc > 3 ) // make sure there are 2 args passed
    {
    ...
    while( !feof(cmdFile) ) // while not EOF
	{
		...
		else if ( a == -7 ) // clear the q
		{
			...
			while ( !isEmptyq(s) ) // while q not empty
			{
				x = deq(s); // remove value from q and get return
				len++; // increase count
				sum += x; // sum values
